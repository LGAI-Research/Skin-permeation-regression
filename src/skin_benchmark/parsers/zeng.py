...
def parse_source(file_path: Path, source_config: SourceConfig, sheet_override: str | None = None):
    """Parse Zeng et al. into the common raw schema."""
    ...
    filtered = table[table[source_record_column].notna()].copy()
    filtered = filtered[~filtered[compound_column].astype(str).str.contains("training set|test set", case=False, na=False)]
    filtered = filtered.reset_index(drop=True)
    note_columns = [column for column in columns if column not in {source_record_column, compound_column, smiles_column, target_column}]
